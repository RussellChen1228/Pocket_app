// Source code from: https://github.com/samarthagarwal/FlutterScreens
...
class ColorLoader4 extends StatefulWidget {
  ...
  ColorLoader4({
    this.dotOneColor = Colors.blueAccent,
    this.dotTwoColor = Colors.green,
    this.dotThreeColor = Colors.redAccent,
    this.dotFourColor = Colors.yellow,
    this.dotFiveColor = Colors.blueAccent,
...
class _ColorLoader4State extends State<ColorLoader4>
  ...
  void initState() {
    ...
    animation_2 = Tween(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: controller,
        curve: Interval(0.1, 0.7, curve: Curves.ease),
      ),
    );
    ...
    animation_4 = Tween(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: controller,
        curve: Interval(0.3, 0.9, curve: Curves.ease),
      ),
    );
        ...
        children: <Widget>[
          ...
          Transform.translate(
            offset: Offset(
              0.0,
              -30 *
                  (animation_5.value <= 0.50
                      ? animation_5.value
                      : 1.0 - animation_5.value),
            ),
            child: new Padding(
              padding: const EdgeInsets.only(right: 8.0),
              child: Dot(
